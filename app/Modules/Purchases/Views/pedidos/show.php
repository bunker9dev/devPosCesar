...
<div class="purchase-summary">
    <div class="summary-item"><label>Proveedor</label><strong><?= htmlspecialchars($pedido['proveedor_nombre']) ?></strong></div>
    <div class="summary-item"><label>Fecha de solicitud</label><strong><?= htmlspecialchars($pedido['fecha_solicitud']) ?></strong></div>
    <div class="summary-item"><label>Estado</label><strong><span class="badge estado-<?= $pedido['estado'] ?>"><?= ucfirst($pedido['estado']) ?></span></strong></div>
    <?php if ($pedido['vencido']): ?>
        <div class="summary-item"><label>⚠️ Esperando</label><strong class="text-warning"><?= $pedido['dias_abierto'] ?> días</strong></div>
    <?php endif; ?>
</div>
...
<div class="card">
    <?php if ($pedido['estado'] === 'borrador' && $canApprove): ?>
...
<div class="card">
    ...
    <table class="table-main">
        <thead>
            <tr><th>Tipo</th><th>Color</th><th>Cantidad</th><th>Unidad</th><th>Nota</th></tr>
        </thead>
        <tbody>
            <?php foreach ($pedido['items'] as $item): ?>
                <tr>
                    <td><?= htmlspecialchars($item['tipo_nombre']) ?></td>
                    <td><?= htmlspecialchars($item['color_nombre']) ?></td>
                    <td><?= $item['cantidad'] ?></td>
                    <td><?= ucfirst($item['unidad']) ?></td>
        ...
        <table class="table-main">
            ...
            <tbody>
                <?php foreach ($pedido['purchases_vinculadas'] as $pu): ?>
                    <tr>
                        <td><a href="<?= BASE_URL ?>/purchases/show?id=<?= $pu['id'] ?>"><?= htmlspecialchars($pu['numero_documento']) ?></a></td>
                        <td><?= htmlspecialchars($pu['fecha']) ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
