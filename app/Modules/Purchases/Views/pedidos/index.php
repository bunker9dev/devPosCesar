            ...
            <?php foreach ($pedidos as $i => $p): ?>
                ...
                <tr data-id="<?= $p['id'] ?>" class="<?= $eliminado ? 'deleted' : ($p['vencido'] ? 'pedido-vencido' : '') ?>">
                    ...
                    <td data-label="Consecutivo">
                        <a href="<?= BASE_URL ?>/pedidos/show?id=<?= $p['id'] ?>"><?= htmlspecialchars($p['consecutivo']) ?></a>
                        <?php if ($p['vencido']): ?>
                        ...
                        <div class="actions">
                            <?php if (!$eliminado): ?>
                                ...
                                <?php if ($canDelete && $p['estado'] === 'borrador'): ?>
                                    <button class="btn-action delete btn-delete-pedido" data-id="<?= $p['id'] ?>" data-consecutivo="<?= htmlspecialchars($p['consecutivo']) ?>">Eliminar</button>
                                <?php endif; ?>
                            <?php elseif ($canRestore): ?>
                                <button class="btn-action restore btn-restore" data-id="<?= $p['id'] ?>" data-url="<?= BASE_URL ?>/pedidos/restore">Restaurar</button>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
